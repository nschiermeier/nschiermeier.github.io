...
The next thing we went over was the **Method of Moments**. These are estimators obtained by replacing population moments by sample moments: $\mu^{\text{\'}}_k = E[x^k] \rightarrow k^{th}$ population moment, $m_k = \frac{1}{n}\sum_{i=1}^nx_i^k \rightarrow k^{th}$ sample mean. Our goal is to estimate $\mu^{\text{\'}}_k$ by $m_k$, and to estimate $\tau(\theta)$, by first expressing it as a function of population moments ($\tau(\theta) = h(\mu^{\text{\'}}_1,...,\mu^{\text{\'}}_k$)), then estimate $\tau(\theta)$ by $\hat{\tau}(\theta) = h(m_1,...,m_k)$
